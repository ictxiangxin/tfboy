...
class SolveEquation:
    ...
    def solve(self, coefficients, b):
        ...
        if len(coefficients_data.shape) == 1:
            _x = tf.Variable(tf.random_normal([coefficients_data.shape[0], 1]))
        else:
            _x = tf.Variable(tf.random_normal([coefficients_data.shape[1], 1]))
